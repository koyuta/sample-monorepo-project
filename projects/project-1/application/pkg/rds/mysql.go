...
import (
	...
	// mysql driver is used only this package.
...
func Connect(user, passwd, host string, port int, dbname string) (*MySQL, error) {
...
func ConnectWithContext(ctx context.Context, user, passwd, host string, port int, dbname string) (*MySQL, error) {
	uri := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?parseTime=true", user, passwd, host, port, dbname)
